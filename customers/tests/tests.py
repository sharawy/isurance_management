...
class CustomerTestCase(TestCase):
    ...
    def test_add_policies_to_customer(self):
        url = reverse('customer-policies', kwargs={"pk": self.customer.pk})
        response = self.client.put(url, data={"policies": self.policy_list})
    ...
    def test_remove_policies_to_customer(self):
        ...
        url = reverse('customer-policies', kwargs={"pk": self.customer.pk})
        response = self.client.delete(url, data={"policies": self.policy_list})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
